...
def downloadImage(imageLink, imagePath):
    if os.path.exists(imagePath):  #Si le fichier existe déja
        ...
        if (resp.status_code == 200):
            ...
            print(getFail("Il y a eu une erreur inconue lors du téléchargement du fichier "+imageLink+" , Le code html est "+getUnderline(str(resp.status_code))))
            return False
    return True
...
def getHTMLPage(HTMLPageLink):
    ...
    resp = requests.get(HTMLPageLink,
                        cookies=cookies, headers=headers)  #On met en place le coockie
    session = HTMLSession()
    if (resp.status_code == 200):
        print(getOkGreen(
            "Téléchargement de la page {0} ".format(HTMLPageLink)))
    return resp.text
...
def getImagesLinksMangakakalot2(HTMLPage):
    ...
    for comp in range(1, 100):
        regex = re.compile('<img.*?src=[\'"]https://s{0}.mkklcdnv{0}.com(.*?)[\'"]'.format(str(comp)))
        for resultat in regex.findall(str(HTMLPage)):
            imagesLinks.append("https://bu.mkklcdnbuv1.com" + resultat)
    return imagesLinks
...
def getMangaList(mdFilesPaths, mangaSavingPath, imageType):
    os.system("clear")
    mangaList = []
    print(getHeader(getBold(getUnderline("Liste des mangas:\n"))))
    for mdFilePath in mdFilesPaths:
        with open(mdFilePath, "r") as file:
            fileText = file.read()
        for line in fileText.split("\n"):
            mangaList.append(line.split("] : ["))
            mangaList[-1][0], mangaList[-1][-1] = mangaList[-1][0][1:], mangaList[
                -1][-1][:
                        -1]  #On éfface le premier caractère de la première chaîne de caractères et le dernier caractère de la dernière chaîne de caractères
            mangaList[-1].append(mangaSavingPath + mangaList[-1][0] + "/" +
                                 imageType + "/")
            print(getBold(str(len(mangaList)) + "-" +
                          mangaList[-1][0]))  #On affiche le Name du scan
    return mangaList
    ...
    while (True):
        try:
            ...
            if (mangaNum < 0 or mangaNum > len(mangaList) - 1):
                print(
                    getFail("Veuillez entrer un Namebre compris entre 1 et " +
                            str(len(mangaList))))
            else:
                break
        except TypeError:
            print(
                getFail("Veuillez entrer un entier compris entre 1 et " +
                        str(len(mangaList)) + "\n"))
        except ValueError:
            print(
                getFail("Veuillez entrer un entier compris entre 1 et " +
    ...
    while (True):
        try:
            ...
            if (start < 0 or end < 0):
                print(
                    getFail(
                        "Les chapitres doivent êtres supérieures ou égal à 0\n"
                    ))
            elif (start > end):
                print(
                    getFail(
                        "Le chapitre de début doit être inférieur à celui de fin\n"
                    ))
            else:
                break
        except TypeError:
